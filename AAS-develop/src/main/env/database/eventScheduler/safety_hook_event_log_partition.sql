create definer = kunsulup@`%` event safety_hook_event_log_partition on schedule
    every '1' month
        starts '2021-10-01 05:00:00'
    enable
    comment '안전고리 이벤트 로그 테이블 파티션 생성 및 삭제'
    do
    BEGIN
        ...
        DECLARE EXIT HANDLER FOR SQLEXCEPTION
            BEGIN
                ...
                INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                    VALUE('safety_hook_event_log_partition', 1, 0, v_txt_call_stack );
            END;
