create definer = kunsulup@`%` event attendance_member_status_log_partition on schedule
    every '1' MONTH
        starts '2021-04-01 05:00:00'
    enable
    comment 'attendance_member_status_log Partition 생성'
    do
    BEGIN
        ...
        DECLARE EXIT HANDLER FOR SQLEXCEPTION
            BEGIN
                ...
                INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                    VALUE('attendance_member_status_log_partition', 1, 0, v_txt_call_stack );
            END;

        SET @table_name = 'attendance_member_status_log';
        SET @interval_expr = 6;
        ...
        IF @exists_cnt = 0 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('attendance_member_status_log_partition', 1, 1, concat( 'create partitions ', @partition_name));

            SELECT concat( 'create partitions ', @partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('attendance_member_status_log_partition', 1, 0, concat( 'cannt create. exists partitions ', @partition_name));
        ...
        IF @exists_cnt = 1 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('attendance_member_status_log_partition', 1, 1, concat( 'drop partitions ', @partition_name));

            SELECT concat( 'drop partitions ', @partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('attendance_member_status_log_partition', 1, 0, concat( 'cannt drop. not exists partitions ', @partition_name));
