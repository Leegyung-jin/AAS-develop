create definer = kunsulup@`%` event gps_location_hist_partition on schedule
    every '1' month
        starts '2021-10-01 05:10:00'
    enable
    comment 'gps_location_hist 로그 테이블 파티션 생성 및 삭제'
    do
    BEGIN
        ...
        SET @table_name = 'gps_location_hist';
        SET @interval_expr = 6;
        ...
        IF @exists_cnt = 0 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('gps_location_hist_partition', 1, 1, concat( 'create partitions ', @partition_name));

            SELECT concat( 'create partitions ', @partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('gps_location_hist_partition', 1, 0, concat( 'exists partitions ', @partition_name));
        ...
        IF @exists_drop_cnt = 1 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('gps_location_hist_partition', 1, 1, concat( 'drop partitions ', @drop_partition_name));

            SELECT concat( 'drop partitions ', @drop_partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('gps_location_hist_partition', 1, 0, concat( 'not exists partitions ', @drop_partition_name));
