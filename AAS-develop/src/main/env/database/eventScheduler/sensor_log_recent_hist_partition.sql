create definer = kunsulup@`%` event sensor_log_recent_hist_partition on schedule
    every '1' day
        starts '2021-10-01 05:12:00'
    enable
    ...
    BEGIN
        ...
        DECLARE EXIT HANDLER FOR SQLEXCEPTION
            BEGIN
                ...
                INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                    VALUE('sensor_log_recent_hist_partition', 1, 0, v_txt_call_stack );
            END;

        SET @table_name = 'sensor_log_recent_hist';
        ...
        IF @exists_cnt = 0 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('sensor_log_recent_hist_partition', 1, 1, concat( 'create partitions ', @partition_name));

            SELECT concat( 'create partitions ', @partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('sensor_log_recent_hist_partition', 1, 0, concat( 'exists partitions ', @partition_name));
        ...
        IF @exists_drop_cnt = 1 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('sensor_log_recent_hist_partition', 1, 1, concat( 'drop partitions ', @drop_partition_name));

            SELECT concat( 'drop partitions ', @drop_partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('sensor_log_recent_hist_partition', 1, 0, concat( 'not exists partitions ', @drop_partition_name));
