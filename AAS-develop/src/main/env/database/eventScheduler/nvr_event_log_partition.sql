create definer = kunsulup@`%` event nvr_event_log_partition on schedule
    every '1' month
        starts '2021-10-01 05:10:00'
    enable
    comment 'nvr_event_log 로그 테이블 파티션 생성 및 삭제'
    do
    BEGIN
        ...
        DECLARE EXIT HANDLER FOR SQLEXCEPTION
            BEGIN
                ...
                INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                    VALUE('nvr_event_log_partition', 1, 0, v_txt_call_stack );
            END;

        SET @table_name = 'nvr_event_log';
        SET @interval_expr = 6;

        SET @limit_date = DATE_FORMAT( DATE_ADD(NOW(), INTERVAL (@interval_expr + 1) MONTH), '%Y-%m-01' );
        SET @partition_name = concat(
                'network_video_recoder_event_p', DATE_FORMAT( DATE_ADD(NOW(), INTERVAL @interval_expr MONTH), '%Y%m' )
            ) ;
        ...
        IF @exists_cnt = 0 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('nvr_event_log_partition', 1, 1, concat( 'create partitions ', @partition_name));

            SELECT concat( 'create partitions ', @partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('nvr_event_log_partition', 1, 0, concat( 'exists partitions ', @partition_name));
            SELECT concat( 'exists partitions ', @partition_name );
        END IF;

        SET @drop_interval_expr = 12;
        SET @drop_partition_name = concat(
                'network_video_recoder_event_p', DATE_FORMAT( DATE_SUB(NOW(), INTERVAL @drop_interval_expr MONTH), '%Y%m' )
            ) ;
        ...
        IF @exists_drop_cnt = 1 THEN
            ...
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('nvr_event_log_partition', 1, 1, concat( 'drop partitions ', @drop_partition_name));

            SELECT concat( 'drop partitions ', @drop_partition_name );
        ELSE
            INSERT INTO ope_event_log (event_name, event_kind, result, result_desc)
                VALUE('nvr_event_log_partition', 1, 0, concat( 'not exists partitions ', @drop_partition_name));
