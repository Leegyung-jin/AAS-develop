...
@AllArgsConstructor
@NoArgsConstructor
@ApiModel(value="UpdateBuildingFloorRequest", description="빌딩 층(구획) 정보 수정 요청")
public class UpdateBuildingFloorRequest {

    @NotBlank(message="층(구획)명은 필수입니다.")
    @ApiModelProperty(notes = "빌딩 층(구획)명", required = true)
    private String floorName;

    @NotNull(message="작업진행 여부는 필수입니다.")
    @ApiModelProperty(notes = "작업 진행여부. 0: 미진행, 1: 진행", required = true)
    ...
    @ApiModelProperty(notes = "층도면 파일. 없으면 삭제 ( 디폴트 층도면 사용 )")
